...
public class ChongqingMaternityAllowanceStrategy implements MaternityAllowanceStrategy {
    ...
    public MaternityAllowanceResponse calculateMaternityAllowance(MaternityAllowanceRequest request) {
        ...
        // 5. 构建响应详情
        List<String> allowanceDetails = new ArrayList<>();
        allowanceDetails.add(String.format("产假期间发放工资: %.2f元", paidMaternityWage));
        allowanceDetails.add(String.format("员工产前12个月月均工资计算补贴金额：%.2f元÷30天×%d天=%.2f元",
                request.getAverageSalaryPast12Months(),
                request.getMaternityLeaveDays(),
                maternityAllowance));
        allowanceDetails.add(String.format("政府发放补贴金额: %.2f元", governmentAllowance));
        allowanceDetails.add(String.format("员工应享受补贴: %.2f元", entitledSubsidy));
        ...
        response.setAllowanceCompensationDetails(allowanceDetails);
    ...
    public String getSupportedCityCode() {
        return CityEnum.CHONGQING.getCode();
    }
}
