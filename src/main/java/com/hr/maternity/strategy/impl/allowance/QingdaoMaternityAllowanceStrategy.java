...
public class QingdaoMaternityAllowanceStrategy implements MaternityAllowanceStrategy {
    ...
    public MaternityAllowanceResponse calculateMaternityAllowance(MaternityAllowanceRequest request) {
        ...
        // a. 单位申报上年度月平均工资计算补贴金额
        BigDecimal unitDeclaredSalary = request.getUnitMonthlyAverageSalary() != null ? request.getUnitMonthlyAverageSalary() : BigDecimal.ZERO;
        ...
        // b. 员工产前12个月月均工资：青岛不纳入比较（仅比较单位申报与政府发放）
        ...
        calculationDetailsList.add(String.format("单位申报上年度月均工资计算补贴金额：%.2f÷30×%d=%.2f元", unitDeclaredSalary, request.getMaternityLeaveDays(), formulaA));
